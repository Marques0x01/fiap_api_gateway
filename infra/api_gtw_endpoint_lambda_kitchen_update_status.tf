resource "aws_api_gateway_method" "kitchen_update" {
  ...
  resource_id   = aws_api_gateway_resource.put_order_status_kitchen.id
  http_method   = "PUT"
  authorization = "NONE"
...
resource "aws_api_gateway_resource" "put_order_status_kitchen" {
  rest_api_id = aws_api_gateway_rest_api.api_gtw.id
  parent_id   = aws_api_gateway_resource.proxy_kitchen.id
  path_part   = "order-status"

  depends_on = [aws_api_gateway_rest_api.api_gtw, aws_api_gateway_resource.proxy_kitchen]
}
...
resource "aws_api_gateway_integration" "lambda_kitchen_put" {
  ...
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:us-east-2:lambda:path/2015-03-31/functions/${var.lambda_kitchen_arn}/invocations"
}
